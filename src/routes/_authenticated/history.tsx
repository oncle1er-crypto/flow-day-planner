...
function HistoryPage() {
  ...
  const stats = useMemo(
    () => computeStats(tasks, from, to, categories),
    [tasks, from, to, categories],
  );
      ...
      <div className="space-y-6 pt-4 pb-4">
        <Tabs value={period} onValueChange={(v) => setPeriod(v as Period)}>
          <TabsList className="w-full">
            ...
            <TabsTrigger value="last_week" className="flex-1 text-xs">
              Précédente
            </TabsTrigger>
            ...
            <div className="pt-2 space-y-2 text-sm">
              {stats.bestDay && (
                <Insight icon={Trophy} tone="success">
                  Meilleur jour : <b>{format(stats.bestDay.date, "EEEE d MMM", { locale: fr })}</b>{" "}
                  ({stats.bestDay.count} terminées)
                </Insight>
              )}
              {stats.topCategory && (
                <Insight icon={TrendingUp} tone="primary">
                  Catégorie la plus active : <b>{stats.topCategory.name}</b> (
                  {stats.topCategory.count} tâches)
                </Insight>
              )}
              {stats.worstCategory && stats.worstCategory.rate < 50 && (
                <Insight icon={TrendingDown} tone="warning">
                  À améliorer : <b>{stats.worstCategory.name}</b> ({stats.worstCategory.rate}%
                  complétées)
                </Insight>
              )}
              <Insight icon={Sparkles} tone="primary">
                {stats.verdict}
              </Insight>
            </div>
          </div>
                ...
                {stats.daily.map((d) => {
                  ...
                  return (
                    <div key={d.key} className="flex-1 flex flex-col items-center gap-1">
                      <div
                        className="w-full h-full flex flex-col justify-end rounded-md bg-muted/40 overflow-hidden relative"
                        style={{ minHeight: 8 }}
                      >
                      ...
                      <span className="text-[10px] text-muted-foreground">
                        {format(d.date, "EE", { locale: fr })[0].toUpperCase()}
                      </span>
                    </div>
                  );
                })}
          ...
          <div className="space-y-2">
            {isLoading ? (
              ...
              <EmptyState
                ...
                description="Pas encore d'activité sur cette période."
              />
            ) : (
...
function computeStats(
  ...
  const inRange = tasks.filter(
    (t) => t.due_date && isWithinInterval(parseISO(t.due_date), { start: from, end: to }),
  );
  const done = inRange.filter((t) => t.status === "done").length;
  const total = inRange.length;
  const completionRate = total ? Math.round((done / total) * 100) : 0;
  const now = new Date();
  const missed = inRange.filter(
    (t) =>
      t.status !== "done" &&
      t.status !== "cancelled" &&
      t.due_date &&
      parseISO(t.due_date) < new Date(now.getFullYear(), now.getMonth(), now.getDate()),
  ).length;
  const pending =
    inRange.filter((t) => t.status !== "done" && t.status !== "cancelled").length - missed;
  ...
  const bestDay = daily.reduce<{ date: Date; count: number } | null>(
    (acc, d) => (d.done > (acc?.count ?? 0) ? { date: d.date, count: d.done } : acc),
    null,
  );
  ...
  const catList = Array.from(byCat.entries()).map(([id, v]) => ({
    id,
    name: v.name,
    count: v.total,
  ...
  const worstCategory =
    catList.filter((c) => c.count >= 2).sort((a, b) => a.rate - b.rate)[0] ?? null;
  ...
  if (completionRate >= 80) verdict = "Excellent rythme ! Vous maîtrisez votre planning.";
    ...
    verdict = "Bonne progression — visez quelques tâches supplémentaires.";
  ...
  const doneList = inRange
    .filter((t) => t.status === "done")
    .sort((a, b) => (b.completed_at ?? "").localeCompare(a.completed_at ?? ""));
  const missedList = inRange.filter(
    (t) =>
      t.status !== "done" &&
      t.status !== "cancelled" &&
      t.due_date &&
...
function MiniStat({
  ...
  return (
    <div className="rounded-xl border border-border bg-card/40 p-3">
      <div className={`h-7 w-7 rounded-lg grid place-items-center ${t} mb-2`}>
