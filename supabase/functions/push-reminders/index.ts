...
async function encryptPayload(
  ...
  const asPublic = concat(
    new Uint8Array([0x04]),
    b64urlDecode(jwk.x as string),
    b64urlDecode(jwk.y as string),
  );
  const uaJwk = {
    kty: "EC",
    crv: "P-256",
...
Deno.serve(async (req) => {
  ...
  const { data: settings, error: sErr } = await sb
    .from("user_settings")
    .select(
      "user_id, notifications_enabled, daily_reminder_enabled, daily_reminder_time, default_reminder_minutes, timezone",
    );
  ...
  if (toSend.length === 0) {
    return new Response(JSON.stringify({ ok: true, sent: 0, candidates: due.length }), {
      headers: { "content-type": "application/json" },
    });
  }
  ...
  return new Response(
    JSON.stringify({ ok: true, sent, dispatched: toSend.length, candidates: due.length }),
    {
      headers: { "content-type": "application/json" },
    },
  );
