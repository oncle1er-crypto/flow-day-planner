...
BEGIN
  ...
  IF v_tasks >= 1 THEN earned := earned || 'first_task'; END IF;
  IF v_tasks >= 10 THEN earned := earned || 'tasks_10'; END IF;
  IF v_tasks >= 50 THEN earned := earned || 'tasks_50'; END IF;
  IF v_tasks >= 100 THEN earned := earned || 'tasks_100'; END IF;
  IF v_habit_logs >= 1 THEN earned := earned || 'first_habit'; END IF;
  IF v_streak_max >= 7 THEN earned := earned || 'streak_7'; END IF;
  IF v_streak_max >= 30 THEN earned := earned || 'streak_30'; END IF;
  IF v_focus_sessions >= 1 THEN earned := earned || 'first_focus'; END IF;
  IF v_focus_minutes >= 60 THEN earned := earned || 'focus_60'; END IF;
  IF v_focus_minutes >= 600 THEN earned := earned || 'focus_600'; END IF;
  IF v_goals >= 1 THEN earned := earned || 'first_goal'; END IF;
  IF v_goals >= 5 THEN earned := earned || 'goals_5'; END IF;
  IF v_level >= 5 THEN earned := earned || 'level_5'; END IF;
  IF v_level >= 10 THEN earned := earned || 'level_10'; END IF;
  IF v_level >= 25 THEN earned := earned || 'level_25'; END IF;
  IF v_tasks >= 1 AND v_focus_sessions >= 1 AND v_habit_logs >= 1 AND v_goals >= 1
    THEN earned := earned || 'all_rounder'; END IF;

  RETURN QUERY
  INSERT INTO public.user_achievements (user_id, achievement_key)
  SELECT uid, k FROM unnest(earned) AS k
  ON CONFLICT (user_id, achievement_key) DO NOTHING
  RETURNING public.user_achievements.achievement_key;
END;
$$;
